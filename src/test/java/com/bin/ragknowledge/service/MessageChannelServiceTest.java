...
class MessageChannelServiceTest {
    ...
    class SaveOrUpdateChannelValidationTest {
        ...
        @Test
        @DisplayName("当渠道类型为空字符串时应抛出IllegalArgumentException")
        void whenChannelTypeIsEmpty_shouldThrowException() {
            IllegalArgumentException exception = assertThrows(
                    IllegalArgumentException.class,
                    () -> messageChannelService.saveOrUpdateChannel("  ", TEST_KB_ID, testEntity)
            );
            assertEquals("渠道类型不能为空", exception.getMessage());
        }
